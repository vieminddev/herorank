  ...
  <div>
    ...
    <p class="lead text-sm mt-2">
      ...
      {:else if calc.profitable}
        Every $1 on ads brings back <span class="font-semibold" style="color: var(--success)">${calc.roas}</span> in sales — and you keep <span class="font-semibold" style="color: var(--success)">${calc.netProfit}</span> after the spend.
      {:else}
        You're spending more than these ads return — <span class="font-semibold" style="color: var(--danger)">${calc.netProfit}</span> once the product margin is in.
      {/if}
    </p>
