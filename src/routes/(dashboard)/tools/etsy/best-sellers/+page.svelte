<script lang="ts">
  ...
  import { Crown, ArrowUpDown } from "lucide-svelte";
  ...
  <div class="card overflow-hidden">
    ...
    <div class="flex items-center justify-between px-5 py-4 border-b border-border">
      <h3 class="text-base font-bold text-text-primary flex items-center gap-2">
        <Crown size={18} class="text-orange" />
      ...
      <table class="w-full">
        <thead>
          <tr class="border-b border-border-light bg-bg-page/50">
            ...
            <th class="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider text-text-muted">
              <span class="flex items-center gap-1 justify-end cursor-pointer hover:text-text-primary">Sales <ArrowUpDown size={10} /></span>
            </th>
            <th class="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider text-text-muted">
        ...
        <tbody>
          {#each MOCK_SHOPS as shop (shop.rank)}
            <tr class="border-b border-border-light hover:bg-bg-page/50 transition-colors cursor-pointer">
              <td class="px-4 py-3 text-sm font-semibold text-text-primary">{shop.rank}</td>
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3 text-sm text-text-primary text-right">{shop.rating}</td>
              <td class="px-4 py-3 text-sm text-text-muted text-right">{shop.opened}</td>
            </tr>
          {/each}
        </tbody>
      </table>
