<script lang="ts">
   ...
   * POST /api/tools/review-request-draft         LLM draft (metered, 1 credit) via callTool
  ...
  const makeDraft = async (order: Order) => {
    ...
    const res = await callTool<Draft>("review-request-draft", {
...
>
  {#snippet controls()}
    ...
    <div class="mt-5 p-3 rounded-lg bg-surface-2 text-[0.8125rem] text-text-secondary leading-relaxed">
  ...
  {:else if !connected}
    <div class="max-w-md mx-auto text-center py-12 animate-fade-in">
      ...
      <p class="lead text-sm mb-6">
        Link your shop once and we'll line up your recent buyers so you can thank them and invite an honest review — read-only and private.
      </p>
