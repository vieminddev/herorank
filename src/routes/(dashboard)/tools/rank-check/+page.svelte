<script lang="ts">
  ...
  interface RankResult {
    ...
    // Present only when the backend is serving fixtures (no live Etsy key). Usually absent.
    sample?: boolean;
    mock?: boolean;
  }

  let listingUrl = $state(page.url.searchParams.get("listing") ?? "");
  ...
  const isSample = $derived(result?.sample === true || result?.mock === true);
...
<ToolPageLayout title="Search Position" description="See where your listing lands for a keyword on Etsy, and watch its spot move as you work on it. Positions are estimated.">
  ...
  {:else if hasSearched && result}
    <div class="animate-fade-in">
      {#if isSample}
        <div class="mb-5"><SampleDataBadge /></div>
      {/if}
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-x-8 gap-y-5">
        <div>
      ...
      {:else}
        <RankChart points={chartHistory.map((d) => ({ date: d.date, rank: d.rank }))} ariaLabel="Search position for {result.keyword} over time" />
      {/if}
    </div>
