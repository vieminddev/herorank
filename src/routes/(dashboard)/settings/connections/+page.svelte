<script lang="ts">
   ...
   *   - no shops    → Connect button → GET /api/connect/etsy/start (server 302 to Etsy)
   *   - has shops   → list of connected shops (name + last calibrated + Disconnect) + "Add another"
   ...
   */
  import { Store, Link2, Plus, Unlink, ShieldCheck, LoaderCircle, CircleAlert, CircleCheck, Info } from "lucide-svelte";
...
<div class="max-w-3xl mx-auto animate-fade-in">
  <div class="mb-8">
    ...
    <p class="text-sm text-text-secondary mt-1">
      Connect your Etsy shops — you can link more than one. We read only your shop's data (read-only) to power real numbers and better estimates.
    </p>
  </div>
  ...
  {#if disconnectError}
    <div class="mb-6 flex items-start gap-2 text-sm text-danger" role="alert">
    ...
    <div class="space-y-3 mb-5">
      {#each shops as shop}
        <div class="card p-5 flex items-center gap-4">
          <div class="w-11 h-11 rounded-xl bg-gradient-to-br from-teal to-teal-dark flex items-center justify-center flex-shrink-0">
            <Store size={20} class="text-white" />
          </div>
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2">
              <h2 class="text-[0.9375rem] font-semibold text-text-primary truncate">
            ...
            <p class="text-xs text-text-muted mt-1">
              Connected {fmtDate(shop.connectedAt)} · Last calibrated {fmtDate(shop.lastCalibratedAt)}
            </p>
          </div>
          <button
            type="button"
            onclick={() => handleDisconnect(shop.shopId)}
          ...
          >
            {#if disconnectingId === shop.shopId}<LoaderCircle size={14} class="animate-spin" /> Disconnecting…{:else}<Unlink size={14} /> Disconnect{/if}
          </button>
        </div>
      {/each}
    </div>
  ...
  <div class="card p-6">
    {#if shops.length}
      <h2 class="text-base font-semibold text-text-primary">Add another shop</h2>
      <p class="text-sm text-text-secondary mt-1">
        Connect a different Etsy shop — sign in to that shop's Etsy account when prompted.
      </p>
      <a
      ...
      >
        <Plus size={15} /> Connect another shop
      </a>
    {:else}
      <h2 class="text-base font-bold text-text-primary">Connect your Etsy shop</h2>
      <p class="text-sm text-text-secondary mt-1">
        Securely link your shop with Etsy — read-only access to transactions, shop details, and listings. No write access.
      </p>
      <a
        href="/api/connect/etsy/start"
        class="mt-4 inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold text-white hover:opacity-90 transition-all"
